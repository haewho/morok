...
| Автоматический архив new/edit/delete/difference | P0 | реализуется | OFF-by-default account policy и allowlist до 256 чатов; новые разрешённые messages и последующие стандартные edit/delete/history updates проходят через bounded encrypted WAL/replay. Есть явный cache-only импорт последних 100 локальных сообщений выбранного чата; более глубокий импорт, полный набор difference/channel-difference и device crash-test впереди |
| Действительные файлы вне обычного кеша | P0 | реализуется | Ручные и автоматические карточки переиспользуют зашифрованные оригиналы, если полные байты уже скачаны; авто-download и полный reference counting впереди |
