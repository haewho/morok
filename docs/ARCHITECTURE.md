...
- `org.morok.memory` / `history` — явно созданные пользователем карточки, снимки, приватное хранение и напоминания. Полный автоматический архив всех событий является отдельным незавершённым этапом.
