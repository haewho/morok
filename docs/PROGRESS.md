...
- `./scripts/check.sh`: upstream lock и 10 submodules, 12 round-video diagnostic domain checks + 7 integration-инвариантов, 35 Memory domain checks, 9 Memory hook/cache-only checks, 6 proxy transaction cases, 34 proxy core checks, appearance mode mapping, settings migration/isolation/archive-policy и strict secret-free profile codec checks, совместимость Python/OpenSSL signer с Java verifier;
