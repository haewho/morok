...
- зашифрованные account-local шаблоны ответов: создание/редактирование/поиск/удаление/clear-all, выбор из меню обычного writable чата и вставка в текущий cursor без удаления черновика или выделенного текста; превышение Telegram message limit отклоняется до изменения поля, а отправка остаётся отдельным штатным нажатием;
...
- reply-template JVM/source checks: UUID/поля/поиск/границы, draft-preserving insertion/spacing/length limit, encrypted account storage, nested logout revocation и доказуемое отсутствие send/network/storage hooks;
...
- на эмуляторе Android 16 APK установлен и холодно запущен; вручную открыты intro, MOROK connection, MOROK Settings, экран переноса, профили приложения, жесты, единая диагностика и безопасность; в Local tools видны новые списки «Reply templates» и «Chat aliases and notes», а до входа оба корректно отказывают без сбоя; диагностика показывает сборку/устройство, состояние direct-прокси, явное отсутствие account-local данных до входа, состояние кружочков и выполняет действие копирования без сбоя; Android shell не имеет доступа к clipboard приложения, точный отчёт проверен pure-Java тестом; экран жестов до входа показывает явное требование авторизации без сбоя навигации; защита экрана добавляет `FLAG_SECURE`, даёт полностью чёрный screenshot, сохраняется после cold restart и корректно снимается; ссылка открывает штатный setup кода-пароля, account-local уведомления до входа показывают явный отказ, переключатель preview кружочков виден и переживает restart. Авторизованные templates, aliases/notes, diagnostics/archive/Memory, double-tap/reaction/swipe, profile apply/Custom/Previous, SAF export/import, реальные notification preview, подтверждение звонка и камера/отправка кружочка ещё не проверены на устройстве.
