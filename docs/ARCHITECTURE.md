...
- `org.morok.appearance` — политика эффектов поверх существующих render paths, bounded device-wide геометрия обычного dialog list (высота и аватары) и локализованная детализация недавнего времени; перечень покрытия указан отдельно.
