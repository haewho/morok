...
| Шифрование, logout, replay/tombstones, лимиты, поиск | P0 | реализуется | Слой Памяти; настраиваются 30/90/180/365 дней, 64/128/256 МиБ автоархива, 1/4/8 МиБ на оригинал и never/Wi-Fi/any-network политика уже готовых кешированных байтов; ручные карточки имеют приоритет. Device/crash/key-loss проверки впереди |
...
| Память: карточка/цитата/файл/заметка/теги/поиск | P0 | реализуется | Native экран, encrypted private store, cache-only повтор оригинала, диагностика хранилища и явный SAF ZIP-экспорт выбранных карточек/уникальных файлов без plaintext temp; обратный импорт и device-проверка впереди |
...
| Copy selection/templates/drafts/chat aliases/notes | P1 | реализуется | Локальные alias + private note имеют account-bound Tink/Keystore store и searchable список. Reply templates локально подставляют `{name}`/`{first_name}`/`{date}`/`{time}`, показывают обязательный preview и вставляются в cursor без auto-send. Отдельный encrypted plain-text draft snapshot привязан к chat/topic и восстанавливается с подтверждением замены. Штатные handles частичного выделения сохранены; guarded-действие вставляет выбранный текст прямо в composer только при разрешённом upstream Copy. Secret/noforwards исключены; alias в основном списке Telegram, formatting и снимки reply/effect/media state впереди |
