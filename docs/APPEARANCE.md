...
Portable validation: `JAVA_HOME=<JDK> tests/settings/run.sh` compiles production repository classes with Java 8 compatibility and checks mode mapping/custom detection/invalid selection, defaults, additive migration, restart, category reset preserving unrelated settings, stable 64-bit account identity isolation, invalid IDs and downgrade refusal. These checks passed under Android Studio JBR 21 on 2026-09-07. They do not establish Android rendering correctness or GPU savings. APK compilation, native runtime, TalkBack, large text, old API fallback, off/on without draft/scroll loss and whole-surface coverage must be reported separately.
