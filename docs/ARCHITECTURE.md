...
- `org.morok.appearance` — политика эффектов поверх существующих render paths и bounded device-wide геометрия обычного dialog list (высота и аватары); перечень покрытия указан отдельно.
