...
| Автоматический архив new/edit/delete/difference | P0 | реализуется | OFF-by-default account policy и allowlist до 256 чатов; новые разрешённые messages и последующие стандартные edit/delete/history updates проходят через bounded encrypted WAL/replay. Старый history import, полный набор difference/channel-difference и device crash-test впереди |
