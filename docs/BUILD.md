...
Выполните `./scripts/build.sh debug`. Без собственных API-данных получается только сборка для разработки интерфейса: экран входа объясняет отсутствие конфигурации, авторизация не заявляется. Debug подписывается локальным стандартным Android debug keystore; если его ещё нет на машине или CI runner, скрипт создаёт новый host-local ключ. Этот ключ не используется для release и не заимствован у Telegram. Перед assemble скрипт удаляет только прежние конечные APK выбранного варианта, сохраняя Gradle caches и промежуточные результаты компиляции: это не даёт incremental packager накапливать многомегабайтные пустоты между ZIP-записями. Проверенный APK копируется в `artifacts/` вместе с SHA256, максимальным межзаписным ZIP-разрывом и сведениями о подписи; разрыв больше 64 KiB отклоняется. AGP при injected ABI оставляет исходный APK в `build/intermediates/apk/afat/debug/`, что учитывает скрипт отчёта.
