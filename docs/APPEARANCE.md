...
The device preferences are local to `morok_device`, current schema 15. `AppearanceSettings` is immutable and typed. An older file receives additive defaults; a save writes the two effect keys, dialog-list density and schema in one SharedPreferences editor operation. Reads do not rewrite preferences. Reset only changes those three keys. A newer schema rejects writes. SharedPreferences `apply()` provides normal Android asynchronous durability, not a synchronous crash-proof transaction; no stronger guarantee is claimed.
...
Native entry: Telegram Settings → MOROK Settings. English and Russian strings live in separate MOROK resource files. The native screen has text search, named appearance modes, reviewed app profiles, Liquid Glass, reduced effects, compact/standard/comfortable dialog-list density, Telegram themes and power-saving shortcuts, category reset, Memory and connection settings links. The separate transfer screen exports/imports reviewed MOROK appearance, round-video and global privacy flags without account/chat/proxy/archive/Memory data. Theme/AMOLED customization beyond Telegram’s existing theme UI remains pending.
...
| `org/telegram/ui/Cells/DialogCell`: `computeHeight` / `getCollapsedHeight` | After upstream chooses the row variant and before dp conversion | Only Default/Folder1/Folder2 dialog lists receive the bounded −8/0/+8 dp offset; content, avatar, sort/search and non-list pickers remain upstream | JVM/source tests; compact/standard/comfortable list/folder/forum/archive/large-font device matrix |
