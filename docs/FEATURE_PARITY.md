...
| Copy selection/templates/drafts/chat aliases/notes | P1 | реализуется | Локальные alias + private note имеют account-bound Tink/Keystore store и searchable список; alias показан в заголовке без изменения server profile/icons. Reply templates вставляются в cursor без auto-send. Отдельный зашифрованный plain-text draft snapshot привязан к chat/topic, не меняет штатный Telegram draft и восстанавливается с подтверждением замены. Secret/Saved исключены; alias в основном списке Telegram, formatting/variables, partial text selection и снимки reply/effect/media state впереди |
